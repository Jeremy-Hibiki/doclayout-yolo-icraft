...
def save_time(network, times, filenames):
    ...
    op_name = np.array(
        [
            network.getOpById(op_id).name.replace("Node", "") if op_id <= max_id else ""
            for op_id in list_keys
        ]
    )
    op_type = np.array(
        [
            network.getOpById(op_id).typeKey().replace("Node", "")
            if op_id <= max_id
            else "icraft::xir::HardOp"
            for op_id in list_keys
        ]
    )
...
def simple_analyze_time(net_dict):
    ...
    for i in range(len(net_dict["op_type"])):
        ...
        if (
            net_dict["op_type"][i] == "icraft::xir::Cast"
            and net_dict["memcpy_time"][i] > cast_thresh
        ):
    ...
    print(
        "Hardop_TotalTime:{:.4f} ms, Hardop_HardTime:{:.4f} ms".format(
            hardop_totaltime, hardop_hardtime
        )
    )
    ...
    for i in range(len(customops["customop_name"])):
        print(
            "Customop:{}, TotalTime:{:.4f}ms, HardTime:{:.4f}ms".format(
                customops["customop_name"][i],
                customops["total_time"][i],
                customops["hard_time"][i],
            )
        )
    ...
    if CENTER_CUSTOMOP:
        ...
        print(
            "icore(NPU)时间:{:.4f}, Device: ['NPU' + {}]".format(
                icore_time, other_customop_list
            )
        )
    else:
...
def calctime_detail(sess, network, name="time_results.xlsx"):
    ...
    result = (
        sess.timeProfileResults()
    )  # 获取时间，[总时间，传输时间, 硬件时间，余下时间]
    ...
    print(
        "Total_TotalTime:{:.4f} ms, Total_MemcpyTime:{:.4f} ms, Total_HardTime:{:.4f} ms, Total_OtherTime:{:.4f} ms".format(
            total_totaltime, total_memcpytime, total_hardtime, total_othertime
        )
    )
    # 简略版耗时分析
