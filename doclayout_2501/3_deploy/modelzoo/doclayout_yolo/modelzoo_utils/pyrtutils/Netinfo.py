...
class Netinfo:
    ...
    def __init__(self, network):
        ...
        if self.mmu:
            mprint(
                "The speedmode or compressFtmp of input model is enabled, The device must enable mmu mode.",
                VERBOSE,
                0,
            )
        else:
            mprint(
                "Neither speedmode nor compressFtmp of input model is enabled, You can decide whether to enable the mmu mode of the device by using the configuration item mmu in the yaml file",
                VERBOSE,
                0,
            )
    ...
    def getNetInfo(self):
        self.fpgaOPlist()
        if self.resize_on:
            self.inp_shape_opid = self.inp_shape_opid + 1
