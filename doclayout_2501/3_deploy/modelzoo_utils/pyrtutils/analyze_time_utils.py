...
def sheet_style(writer, max_index):
    ...
    # 设置填充区域
    fill_style1 = PatternFill(
        start_color="C4DFA3", end_color="C4DFA3", fill_type="solid"
    )
    cell_range1 = worksheet["A1:C30"]
    fill_style2 = PatternFill(
        start_color="D1EAF0", end_color="D1EAF0", fill_type="solid"
    )
...
def calculate_totaltime(writer, df, sheet_name, startcol=0, startrow=0):
    ...
    title_row.to_excel(
        writer, sheet_name=sheet_name, startcol=startcol, startrow=startrow
    )
    df_total.to_excel(
        writer, sheet_name=sheet_name, startcol=startcol, startrow=startrow + 2
    )
    writer.save()
...
def calculate_hardop_time(writer, df, sheet_name, startcol=12, startrow=15):
    ...
    columns_to_keep = [
        col
        for col in op_type
        if col.split("::")[0] == "customop" or col.split("::")[-1] == "HardOp"
    ]
    df_hardop = df_types.loc[
        columns_to_keep, ["total_time", "memcpy_time", "hard_time", "other_time"]
    ]
...
def calculate_detailtime(writer, df, sheet_name, startcol=12, startrow=25):
    ...
    for col in op_type:
        ...
        if a == "customop":
            ...
            #     pre_customop_time += df.loc[df["op_type"]==col, "total_time"].values[0]
                ...
                other_customop_time += df.loc[
                    df["op_type"] == col, "total_time"
                ].values[0]
        else:
            continue
    ...
    # 获取输入icore的时间
    if IMK:
        icore_in_time = df.loc[
            df["op_type"] == "customop::ImageMake", "hard_time"
        ].values[0]
    else:
        icore_in_time = df.loc[
            df["op_type"] == "icraft::xir::HardOp", "memcpy_time"
        ].sum()

    # 获取icore输出的时间
    if POST:
        icore_out_time = df.loc[
            df["op_type"].str.split("::").str[-1].str[-4:] == "Post", "total_time"
        ].values[0]
    else:
        filtered_cast_df = df[
            (df["op_type"] == "icraft::xir::Cast") & (df["memcpy_time"] > cast_thresh)
        ]
    ...
    if IMK:
        ...
        icore_time = (
            df.loc[df["op_type"] == "icraft::xir::HardOp", "total_time"].sum()
            - icore_in_time
        )
    ...
    # 纯CPU端算子耗时
    df_cpu = df[
        (df["op_type"] != "icraft::xir::HardOp")
        & (~df["op_type"].str.startswith("customop::"))
    ]
    if POST:
    ...
    df_res["耗时瓶颈"] = [
        "=INDEX(A4:A8,MATCH(MAX(B4:B8),B4:B8,0))",
        "=INDEX(A4:A8,MATCH(MAX(B4:B8),B4:B8,0))",
    ]
    ...
    df_res_print = pd.DataFrame(
        [delay_time, fps], index=row_names_res, columns=["results"]
    )
    df_res_print["耗时瓶颈"] = [
        df_detailtime["time(ms)"].idxmax(),
        df_detailtime["time(ms)"].idxmax(),
    ]
    ...
    df_detailtime.to_excel(
        writer, sheet_name=sheet_name, startcol=startcol, startrow=startrow + 2
    )
    title_row2 = pd.DataFrame(index=["单帧延时、吞吐率分析结果如下:"])
    title_row2.to_excel(
        writer, sheet_name=sheet_name, startcol=startcol, startrow=startrow + 10
    )
    df_res.to_excel(
        writer, sheet_name=sheet_name, startcol=startcol, startrow=startrow + 12
    )
    writer.save()
...
def analyze_time(filepath, res_root="./res/", mergeOps=True):
    ...
    if mergeOps:
        ...
        for sheet_name, df_origin in input_data.items():
            ...
            # 绘制各类hardop算子的耗时直方图
            draw_total_time(
                op_name, df_sum, filename, respath, "各类hardop算子的耗时直方图"
            )

            # 绘制所有算子的耗时直方图
            df_filtered = df[
                df["op_type"] == "icraft::xir::HardOp"
            ]  # 数据筛选,只保留op_type=hard_op
            op_id = df_filtered["op_id"]
            draw_per_op_time(
                op_id, df_filtered, filename, respath, "所有hardop算子的耗时直方图"
            )
