...
def pred_one_image(img_path, model_path, test_size):
    ...
    # show results
    result_image = vis(
        img_raw, boxes=pred[:, :4], scores=pred[:, 4], cls_ids=pred[:, 5], conf=conf_thres, class_names=COCO_CLASSES
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", type=str, default="../2_compile/fmodel/yolov10n_640x640.pt", help="torchscript model path"
    )
